...
if left_images and right_images:
    ...
    R = np.eye(3, dtype=np.float64)  # Use np.float64
    T = np.array([1, 0, 0], dtype=np.float64)  # Use np.float64
    ...
    rectified_left = cv2.remap(undistorted_left, map1x, map1y, cv2.INTER_LINEAR)
    rectified_right = cv2.remap(undistorted_right, map2x, map2y, cv2.INTER_LINEAR)

    # Mostrar as imagens retificadas
    cv2.imshow('Left Image - Rectified', rectified_left)
    cv2.imshow('Right Image - Rectified', rectified_right)

    # Configurar callbacks de mouse para desenhar as linhas epipolares nas imagens retificadas
    left_params = {'points': [], 'image': rectified_right, 'F': F, 'is_left': True, 'window_name': 'Right Image - Rectified'}
    right_params = {'points': [], 'image': rectified_left, 'F': F, 'is_left': False, 'window_name': 'Left Image - Rectified'}
